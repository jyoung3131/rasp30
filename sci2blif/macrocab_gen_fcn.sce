...
function Start_MC_design_callback()
    ...
    fd_r = mopen("$FPAAHOME/rasp30/vpr2swcs/block_list",'r');block_list=mgetl(fd_r);mclose(fd_r);  // Default value: frame. 
    ...
    file_list=listfiles("$FPAAHOME/rasp30/xcos_blocks/*.sci");
    l_file_list=size(file_list,1);
    for ii=1:l_file_list
        if file_list(ii) == "$FPAAHOME/rasp30/xcos_blocks/"+macrocab_name+".sci" then messagebox('Please change the name of macro-cab block.', "Macroblock name error", "error"); abort; end
    end
    
    temp_string="$FPAAHOME/RASP_Workspace/"+folder_name;
    mkdir(temp_string);
    cd(temp_string);
    unix_g("cp $FPAAHOME/rasp30/sci2blif/xcos_ref/macrocab_generation/macrocab_xcosref_30_30a.xcos "+macrocab_name+".xcos");
...
function Generate_MC_callback()
    ...
    dir_frame ="$FPAAHOME/rasp30/vpr2swcs/macroblk_generation/frame/";
...
//    disp(match_ele);
    ...
    //////////////////////////////////
    fd_w= mopen("$FPAAHOME/rasp30/sci2blif/block_info/bi_"+macrocab_name+".sci",'wt');
    ...
    mblif_xcos_r=[""];
    ele_index_r=[""];
    fd_r = mopen("$FPAAHOME/rasp30/sci2blif/block_info/bi_"+macrocab_name+".sci",'r');
    for i=1:7
    ...
    fd_w= mopen ("$FPAAHOME/rasp30/xcos_blocks/"+macrocab_name+".sci",'wt');
    ...
    //////////////////////////////////////////////
    fd_w= mopen ("$FPAAHOME/rasp30/sci2blif/rasp_design_added_blocks/"+macrocab_name+".sce",'wt');
    ...
    fd_w= mopen ("$FPAAHOME/rasp30/sci2blif/sci2blif_added_blocks/"+macrocab_name+".sce",'wt');
    ...
    dir_py="$FPAAHOME/rasp30/vpr2swcs/";
    dir_arch="$FPAAHOME/rasp30/vpr2swcs/arch/";
    ...
    fd_w = mopen("$FPAAHOME/rasp30/vpr2swcs/block_list",'wt');mputl(block_list,fd_w);mclose(fd_w);
    
    unix_w("cp "+macrocab_name+".xcos $FPAAHOME/rasp30/sci2blif/xcos_ref/macrocab_generation/");
