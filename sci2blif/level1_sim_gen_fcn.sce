...
function Start_L1_sim_callback()
    ...
    file_list=listfiles("$FPAAHOME/rasp30/xcos_blocks/*_c.sci");
    l_file_list=size(file_list,1);
    for ii=1:l_file_list
        if file_list(ii) == "$FPAAHOME/rasp30/xcos_blocks/"+L1_sim_name+"_c.sci" then messagebox('A Sim file with the same name exists. Please check it.', "Level 1 sim name error", "error"); abort; end
    end
...
function Gen_L1_sim_callback()
    ...
    unix_w("cp ./"+L1_sim_name+"_c.sci $FPAAHOME/rasp30/xcos_blocks/"+L1_sim_name+"_c.sci");
