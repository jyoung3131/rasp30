...
function Generate_MB_callback()
    ...
    file_list=listfiles("$FPAAHOME/rasp30/sci2blif/block_info/*.sci");
    l_file_list=size(file_list,1);
    for ii=1:l_file_list
        if file_list(ii) == "$FPAAHOME/rasp30/sci2blif/block_info/"+para_abs_name+".sci" then messagebox('The block does not exist. Please check the block name.', "an error", "error"); abort; end
    end
