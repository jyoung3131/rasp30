...
if (blk_name.entries(bl) =='div_by_n') then
    ...
    mclose(fd_w2);
    unix_w(' perl $FPAAHOME/rasp30/vtr_release/vtr_flow/scripts/run_vtr_flow.pl $FPAAHOME/rasp30/sci2blif/benchmarks/verilog/div_by_n.v $FPAAHOME/rasp30/vtr_release/vpr_rasp3/rasp3_arch.xml -ending_stage scripts -no_mem');
    unix_w('pwd ') ;
    unix_w('cp temp/div_by_n.pre-vpr.blif  ./'+digifl+' -rf'); 
    //unix_w('cp $FPAAHOME/rasp30/vtr_release/vtr_flow/temp/div2.pre-vpr.blif  $FPAAHOME/rasp30/vtr_release/vtr_flow/div2.blif -rf'); 
    unix_w('pwd ') ;
    //disp("Copied files");
