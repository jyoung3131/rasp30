...
if (blk_name.entries(bl) =='generic_dig') then
    ...
    mclose(fd_w2);
    unix_w('perl $FPAAHOME/rasp30/vtr_release/vtr_flow/scripts/run_vtr_flow.pl '+ scs_m.objs(bl).model.opar(1) +' $FPAAHOME/rasp30/vpr2swcs/arch/rasp3_arch.xml -ending_stage scripts -no_mem'); // $FPAAHOME/rasp30/vtr_release/vpr/ARCH/arch.xml
    unix_w('pwd ') ;
    unix_w('cp temp/'+blname+'.pre-vpr.blif  ./'+blname+ '.blif -rf'); 
    unix_g('pwd ') ;
    unix_w('bash $FPAAHOME/rasp30/vtr_release/vtr_flow/genblif.sh '+ blname) ;
